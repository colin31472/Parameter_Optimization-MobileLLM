...
class GPTBlock(nn.Module):
    ...
    def forward(self, logits):
        ...
        adn_logits = self.ln1(logits + att_logits) ## Residual 적용할 때  alpha * logits + (1 - alpha) * att_logits 실험
...
def save_model(model, optimizer, step, loss, best_eval_loss, save_dir='checkpoints'):
    ...
    path = os.path.join(save_dir, f'Layer_Sharing_model_step_{step}.pt')
...
checkpoint_path = 'checkpoints/Layer_Sharing_model_step_10.pt'

# Hyperparameters
lr = 2e-3
epochs = 1
eval_steps = 500  # 100 steps마다 평가 수행
...
# checkpoint_path = 'checkpoints/Baseline_model_epoch_0.pt'
